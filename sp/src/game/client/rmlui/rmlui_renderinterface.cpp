...
void RmlUIRenderInterface::BlitLayerToPostprocessPrimary(Rml::LayerHandle layer_handle)
{
    CMatRenderContextPtr pRenderContext(materials);

    ITexture* source = renderLayers.GetLayer(layer_handle);
    ITexture* destination = renderLayers.GetPostprocessPrimary();

    pRenderContext->SetRenderTarget(source);
    pRenderContext->CopyRenderTargetToTexture(destination);
}

void RmlUIRenderInterface::RenderFilters(Rml::Span<const Rml::CompiledFilterHandle> filter_handles)
{
    CMatRenderContextPtr pRenderContext(materials);
...
void RmlUIRenderInterface::BeginFrame()
{
    ...
    renderLayers.BeginFrame(ScreenWidth(), ScreenHeight());

    pRenderContext->SetRenderTarget(renderLayers.GetTopLayer());
    pRenderContext->ClearColor3ub(0, 0, 0); // Black (R, G, B)
...
void RmlUIRenderInterface::EndFrame()
{
    ...
    ITexture* fbActive = renderLayers.GetTopLayer();
    ITexture* fbPostprocess = renderLayers.GetPostprocessPrimary();

    pRenderContext->SetRenderTarget(fbActive);
    pRenderContext->CopyRenderTargetToTexture(fbPostprocess);

    pRenderContext->SetRenderTarget(0);

    renderLayers.EndFrame();
...
{
    const Rml::LayerHandle layer_handle = renderLayers.PushLayer();

    CMatRenderContextPtr pRenderContext(materials);
    pRenderContext->SetRenderTarget(renderLayers.GetLayer(layer_handle));
...
void RmlUIRenderInterface::PopLayer()
{
    ...
    ITexture* pTopLayer = renderLayers.GetTopLayer();
    pRenderContext->SetRenderTarget(pTopLayer ? pTopLayer : NULL);
}

void RmlUIRenderInterface::CompositeLayers(Rml::LayerHandle source, Rml::LayerHandle destination, Rml::BlendMode blend_mode, Rml::Span<const Rml::CompiledFilterHandle> filters)
{
    using Rml::BlendMode;
    ...
    // TODO: If we have BlendMode::Replace and no filters or mask then we can just blit directly to the destination.
    ...
    pRenderContext->SetRenderTarget(renderLayers.GetLayer(destination));

    // Render material here
    pRenderContext->DrawScreenSpaceQuad(nullptr);

    if (destination != renderLayers.GetTopLayerHandle())
    {
        ITexture* pTopLayer = renderLayers.GetTopLayer();
        pRenderContext->SetRenderTarget(pTopLayer ? pTopLayer : NULL);
    }
...
RmlRenderLayerStack::RmlRenderLayerStack()
{
    fbPostProcess.resize(4);
}
...
Rml::LayerHandle RmlRenderLayerStack::PushLayer()
{    
    Assert(layersSize <= (int)fbLayers.size());

    CMatRenderContextPtr pRenderContext(materials);

    if (layersSize == (int)fbLayers.size())
    {
        ITexture* fb = materials->CreateRenderTargetTexture(width, height, RT_SIZE_OFFSCREEN, IMAGE_FORMAT_RGBA8888, MATERIAL_RT_DEPTH_SHARED);
        fbLayers.push_back(fb);
    }
...
void RmlRenderLayerStack::PopLayer()
{
    Assert(layersSize > 0);
    layersSize--;
}

ITexture* RmlRenderLayerStack::GetLayer(Rml::LayerHandle layer) const
{
    Assert((size_t)layer < (size_t)layersSize);
    return fbLayers[layer];
}

ITexture* RmlRenderLayerStack::GetTopLayer() const
{
    return GetLayer(GetTopLayerHandle());
}

Rml::LayerHandle RmlRenderLayerStack::GetTopLayerHandle() const
{
    Assert(layersSize > 0);
    return static_cast<Rml::LayerHandle>(layersSize - 1);
}

void RmlRenderLayerStack::SwapPostprocessPrimarySecondary()
{
    std::swap(fbPostProcess[0], fbPostProcess[1]);
}

void RmlRenderLayerStack::BeginFrame(int new_width, int new_height)
{
    Assert(layersSize == 0);
...
void RmlRenderLayerStack::EndFrame()
{
    Assert(layersSize == 1);
    PopLayer();
}

void RmlRenderLayerStack::DestroyFramebuffers()
{
    AssertMsg(layersSize == 0, "Do not call this during frame rendering, that is, between BeginFrame() and EndFrame().");

    for (ITexture* fb : fbLayers)
    {
        if (fb)
            fb->DecrementReferenceCount();
    }

    fbLayers.clear();

    for (ITexture* fb : fbPostProcess)
    {
        if (fb)
            fb->DecrementReferenceCount();
    }
...
ITexture* RmlRenderLayerStack::EnsureFramebufferPostprocess(int index)
{
    Assert(index < (int)fbPostProcess.size());
    ITexture*& fb = fbPostProcess[index];

    CMatRenderContextPtr pRenderContext(materials);

    if (!fb)
        fb = materials->CreateRenderTargetTexture(width, height, RT_SIZE_OFFSCREEN, IMAGE_FORMAT_RGBA8888, MATERIAL_RT_DEPTH_SHARED);

    fb->IncrementReferenceCount();
